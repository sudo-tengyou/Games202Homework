            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    Eigen::Array3f Le(images[i][index + 0], images[i][index + 1],
                                      images[i][index + 2]);
                    auto omiga = CalcArea(x, y, width, height);

                    for (int l = 0; l <= SHOrder; l++) {
                        for (int m = -l; m <= l; m++) {
                            auto shValue = sh::EvalSH(l, m, Eigen::Vector3d(dir.x(), dir.y(), dir.z()).normalized());
                            SHCoeffiecents[sh::GetIndex(l, m)] += Le * shValue * omiga;
                        }
                    }
                }
            }
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            auto shFunc = [&](double phi, double theta) -> double {
                ...
                {
                    bool visibility = scene->rayIntersect(Ray3f(v, wi));
                    return !visibility && H > 0 ? H : 0;
                }
            };
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            std::cout << "computed interreflection light sh coeffs" << std::endl;
        }
